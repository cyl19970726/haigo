...
| BFF  | `pnpm --filter @haigo/bff start:dev` | 基于 Node 18+ 原生 `--watch` + `--loader ts-node/esm`，保留装饰器元数据。 |
| Shared | `pnpm --filter @haigo/shared build --watch` | 可选：若频繁修改 DTO，可临时开启 watch（需手动在 `package.json` 添加 watch 脚本）。 |

> **注意**：Node 原生 watch 仍属实验特性，若遇到监听中断，可改用 `pnpm --filter @haigo/bff exec ts-node-dev --respawn --esm --transpile-only src/main.ts` 作为兜底方案。
