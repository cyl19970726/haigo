...
首次启用 W1（Staking）前请执行 Prisma 迁移创建缓存表：
```bash
...
```
注：为降低阻断，BFF 在表缺失时会打印 warn 并跳过持久化（Prisma `P2021` 容错），但请尽快完成迁移以启用监听与聚合。
...
- **启动命令**：
  ...
  ```
- **关键环境变量**：
  - BFF：`DATABASE_URL=postgres://haigo:haigo@localhost:5433/haigo`、`HASURA_URL=http://localhost:8080`、`HASURA_ADMIN_SECRET=haigo-secret`（若启用）、`DIRECTORY_CACHE_TTL_MS=30000`（可选）、`APTOS_INDEXER_URL=https://api.testnet.aptoslabs.com/v1/graphql`。
