...
- [HaiGo 架构说明（PoC v0）](#table-of-contents)
  ...
  - [8. 演进路线](./8-演进路线.md)
  - [9. 附件](./9-附件.md)
  - [10. 场景化端到端数据流](./10-场景化端到端数据流.md)
    - [10.1 场景总览](./10-场景化端到端数据流.md#101-场景总览)
    - [10.2 R1 — 商家/仓库注册事件同步（现网）](./10-场景化端到端数据流.md#102-r1--商家仓库注册事件同步现网)
    - [10.3 R2 — 档案哈希校验（现网）](./10-场景化端到端数据流.md#103-r2--档案哈希校验现网)
    - [10.4 家庭仓质押与存储费设置（规划）](./10-场景化端到端数据流.md#104-家庭仓质押与存储费设置规划)
    - [10.5 商家浏览家庭仓 Listing（选择仓）（规划）](./10-场景化端到端数据流.md#105-商家浏览家庭仓-listing选择仓规划)
    - [10.6 O1 — 订单创建与链上签署（初版）](./10-场景化端到端数据流.md#106-o1--订单创建与链上签署初版)
    - [10.7 家庭仓订单收件箱（可见性）（规划）](./10-场景化端到端数据流.md#107-家庭仓订单收件箱可见性规划)
    - [10.8 O2 — 仓储出库与媒体上传（规划）](./10-场景化端到端数据流.md#108-o2--仓储出库与媒体上传规划)
    - [10.9 商家评价（规划）](./10-场景化端到端数据流.md#109-商家评价规划)
    - [10.10 补偿与监控](./10-场景化端到端数据流.md#1010-补偿与监控)
    - [10.11 场景化 Anchor 设计与任务拆分](./10-场景化端到端数据流.md#1011-场景化-anchor-设计与任务拆分)
