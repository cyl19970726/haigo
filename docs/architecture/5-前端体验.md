...
| 质押/信用面板 | `apps/web/app/(warehouse)/staking/page.tsx`（未来目标路径） | 🚧 | S1 |
...
| 场景 | Anchor（未来目标路径） | 说明 |
|------|------------------------|------|
| O1 订单草稿/签署 | `apps/web/features/orders/create/CreateOrderView.tsx`、`useOrderDraft.ts` | 调用 `POST /api/orders/drafts` 保存草稿，监听 `OrdersEventListener` 状态；需复用 `useWalletContext` 签署交易 |
...
| S1 质押 & 信用 | `apps/web/features/staking/StakingDashboard.tsx`、`apps/web/features/credit/CreditLeaderboard.tsx` | 聚合 BFF `/api/staking/summary` / `/api/credit/board`，展示余额、锁仓、信用分布 |
