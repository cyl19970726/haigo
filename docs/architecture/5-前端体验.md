...
| 仓库目录（Seller） | `apps/web/app/(seller)/warehouses/page.tsx:1`、`apps/web/features/directory/useWarehouseDirectory.ts:1`、`WarehouseCard.tsx:1`、`WarehouseFilters.tsx:1` | ✅ 初版 | L1 |
...
## 5.4 L1 — 家庭仓目录（已实现）
- **页面容器**：`apps/web/app/(seller)/warehouses/page.tsx:1-79` 渲染目录页头、筛选组件、加载/错误占位与卡片网格，并在页脚提供分页切换及缓存提示。
- **数据 Hook**：`useWarehouseDirectory`（`apps/web/features/directory/useWarehouseDirectory.ts:1-143`）管理筛选条件、分页、加载状态与错误处理；内部调用 `fetchWarehouseDirectory`（`apps/web/lib/api/directory.ts:1-74`），支持 AbortController、缓存命中标记以及默认页面大小（12）。
- **筛选组件**：`WarehouseFilters`（`apps/web/features/directory/WarehouseFilters.tsx:1-79`）提供关键词、最低评分、最高费率、服务区域及排序控制，`Reset filters` 按钮会恢复默认值（available-only + score desc）。
- **展示卡片**：`WarehouseCard`（`apps/web/features/directory/WarehouseCard.tsx:1-79`）展示名称、地址、质押分、信用容量、存储费率、服务区域与 CTA（跳转 `/orders/new?warehouse=0x...`）。
- **Dashboard 入口**：`apps/web/app/dashboard/seller/page.tsx:1-28` 在卖家工作台提供 “Browse directory” 卡片，串联 L1 → O1。`CreateOrderView`（`apps/web/features/orders/create/CreateOrderView.tsx:97-121`）新增 `useSearchParams()` 逻辑以解析 `?warehouse=` 预选仓库。
