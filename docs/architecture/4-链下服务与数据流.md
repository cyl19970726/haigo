...
| 订单监听模块 | `apps/bff/src/modules/orders/orders.module.ts`（未来目标路径） | 🚧 | O1、O2 |
...
| 场景节点 | Anchor | 职责 |
|----------|--------|------|
| `OrdersDraftController` | `apps/bff/src/modules/orders/orders.controller.ts`（未来目标路径） | `POST /api/orders/drafts` 保存草稿、返回 `recordUid` 与签名载荷 |
| `OrdersService.createDraft` | `apps/bff/src/modules/orders/orders.service.ts`（未来目标路径） | 校验仓库/卖家注册状态、调用 Prisma 写入 `orders` 草稿表 |
| `OrdersEventListener` | `apps/bff/src/modules/orders/orders-event-listener.service.ts`（未来目标路径） | 监听 `OrderCreated` 等事件，结合 Fullnode 数据更新订单状态 |
| `OrdersTimelineRepository` | `apps/bff/src/modules/orders/orders.repository.ts`（未来目标路径） | 维护时间线、媒体引用，与 `MediaAsset` 表关联 |
