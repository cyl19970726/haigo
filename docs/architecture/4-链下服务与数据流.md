...
| `DirectoryModule` | `apps/bff/src/modules/directory/directory.module.ts:1`、`directory.controller.ts:1`、`directory.service.ts:1`、`directory.repository.ts:1`、`hasura.client.ts:1` | ✅ 初版 | L1 |
...
| 质押模块 | `apps/bff/src/modules/staking/staking.module.ts:1` / `staking.controller.ts:1` / `staking.listener.ts:1` / `staking.repository.ts:1` | ✅ 初版 | W1 |
...
- **Controller**：`GET /api/warehouses`（`apps/bff/src/modules/directory/directory.controller.ts:1-49`）解析 `available/minScore/maxFeeBps/area/q/sort/page/pageSize` 查询参数，返回 `{ data, meta }` 结构，元数据包含分页、缓存命中与生成时间。
- **Service**：`DirectoryService`（`directory.service.ts:10-40`）封装指标打点（`directory_request_total`、`directory_cache_hit_total`、`directory_error_total`、`directory_request_latency_ms`）与错误日志，统一调用仓储层。
- **Repository**：`DirectoryRepository`（`directory.repository.ts:1-220`）负责：
  - 读取 `accounts`、`staking_positions`、`storage_fees_cache`（Prisma）并与 Hasura 画像合并。
  - 在内存维护 `Map<string, CacheEntry>`，默认 TTL 30s（`DIRECTORY_CACHE_TTL_MS` 可调）。
  - 支持筛选（可用性、最低评分、最高费率、区域、关键词）与排序（评分、费率、容量、最近更新时间）。
  - 自动回退：Hasura 查询失败只打印 `warn`，保留 Prisma 数据；缺失费率/评分时仍返回记录。
- **HasuraClient**：`apps/bff/src/modules/directory/hasura.client.ts:1-84`（可选）封装 `warehouse_profiles` 查询，支持 `x-hasura-admin-secret` 与匿名角色，错误时记录 warn。
- **配置项**：`configuration.ts` 新增 `hasuraAdminSecret`、`directory.cacheTtlMs`；环境变量 `DIRECTORY_CACHE_TTL_MS` 控制缓存时长。
- **指标**：通过 MetricsModule 暴露 Prometheus 文本，新增 `directory_request_total`、`directory_cache_hit_total`、`directory_error_total`、`directory_request_latency_ms`。
...
### 4.9.1 模块与接口
- StakingController：
  - GET `/api/staking/:warehouseAddress` → { stakedAmount, minRequired, feePerUnit }（优先链上视图，失败回落缓存）
- StakingService：
  - 通过 Fullnode REST `/v1/view` 调用 `${MODULE}::staking::{get_stake,get_storage_fee}`；异常回落仓库缓存
- StakingListener：
  - 订阅 `StakeChanged` / `StorageFeeUpdated`；维护游标与退避。事务哈希兜底规划中。
- StakingRepository：
  - `staking_positions` / `storage_fees_cache` upsert + 查询。

### 4.9.2 事件与映射
- 事件类型（type 字符串示例）：
  - `${MODULE}::staking::StakeChanged`
  - `${MODULE}::staking::StorageFeeUpdated`
- 映射：
  - StakeChanged → { warehouse, delta, new_amount, timestamp } → `staking_positions.staked_amount`
  - StorageFeeUpdated → { warehouse, fee_per_unit, timestamp } → `storage_fees_cache.fee_per_unit`

### 4.9.3 指标与补偿
- 指标：`staking_listener_last_version`, `staking_listener_error_total`
- 补偿：定期回扫缺失游标区间，重放事件以修复缓存。

### 4.9.4 数据模型（Prisma / Postgres）
```prisma
model StakingPosition { warehouseAddress String @id @map("warehouse_address") stakedAmount BigInt @map("staked_amount") lastTxnVersion BigInt? @map("last_txn_version") lastEventIndex BigInt? @map("last_event_index") updatedAt DateTime @updatedAt @map("updated_at") @@index([lastTxnVersion, lastEventIndex]) @@map("staking_positions") }
model StorageFeeCache { warehouseAddress String @id @map("warehouse_address") feePerUnit Int @map("fee_per_unit") lastTxnVersion BigInt? @map("last_txn_version") lastEventIndex BigInt? @map("last_event_index") updatedAt DateTime @updatedAt @map("updated_at") @@index([lastTxnVersion, lastEventIndex]) @@map("storage_fees_cache") }
```

### 4.9.5 环境变量（Staking 专用）
- `STAKING_INGESTOR_INTERVAL_MS`（默认继承 ingestion.pollingIntervalMs=30000）
- `STAKING_INGESTOR_PAGE_SIZE`（默认 25）
- `STAKING_INGESTOR_MAX_PAGES_PER_TICK`（默认 1）
- `STAKING_INGESTOR_START_FROM_LATEST`（默认 true）
- `STAKING_INGESTOR_BACKFILL_OFFSET_VERSIONS`（默认 0）
- 共享：`NEXT_PUBLIC_APTOS_MODULE`、`APTOS_INDEXER_URL`、`APTOS_NODE_API_URL`、`APTOS_NODE_API_KEY`

## 4.10 共享类型（Staking DTO — 规划）
为支持 W1 场景，统一共享 DTO（最终将落地于 `packages/shared/src/dto/staking.ts`）。

### 4.10.1 StakeChanged（质押变更）
- Move 事件：`{ warehouse: address, delta: u64, new_amount: u64, timestamp: u64 }`
- Type 名称：`${MODULE}::staking::StakeChanged`
- DTO（ts, planned）：
```ts
...
### 4.10.2 StorageFeeUpdated（存储费调整）
- Move 事件：`{ warehouse: address, fee_per_unit: u64, timestamp: u64 }`
- Type 名称：`${MODULE}::staking::StorageFeeUpdated`
- DTO（ts, planned）：
```ts
...
> **落地状态**：本节所述模块已提供最小可用实现；StakingService 视图优先，监听器缓存回落，指标输出到 `/metrics`。
