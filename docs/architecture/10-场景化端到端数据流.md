...
| L1 | 家庭仓 Listing 选择 | 商家 | directory | 🚧 规划中 |
...
sequenceDiagram
  ...
  loop Polling
    ...
    BFF->>DB: 更新 staking_positions；允许设置 storage_fee
  end
  FE->>BFF: POST /api/staking/storage-fee {fee}
  BFF->>DB: 更新仓库费率
  BFF-->>FE: 成功
```

## 10.5 商家浏览家庭仓 Listing（选择仓）（规划）
...
sequenceDiagram
  ...
  FE->>BFF: GET /api/warehouses?available=true
  BFF->>Hasura: 聚合仓库资料、质押容量、费率
  Hasura-->>BFF: 列表数据
  BFF->>DB: 读取附加评分/媒体样本（可选）
  BFF-->>FE: 返回可选家庭仓列表（支持筛选/排序）
```
...
- 监控：`GET /metrics` 暴露 `order_listener_last_version`（gauge）、`order_listener_error_total`（counter）。
