...
- [x] Integrate BFF media storage pipeline (AC: 3)
  - [x] 调用 `POST /api/media/uploads`（包含 `record_uid`、stage、hash`），并确认 BFF 返回值与本地 BLAKE3 匹配后再继续。 [Source: docs/architecture/4-链下服务与数据流.md#4-7-2-媒体存证与上传接口-规划]
  - [x] 将响应中的路径、大小、上传者写入本地状态，为后续 `order_media_assets` ingestion 标记 `matchedOffchain=true` 做准备。 [Source: docs/architecture/4-链下服务与数据流.md#4-2-数据模型-prisma-postgres]
...
- [x] Deliver `/api/media/uploads` backend pipeline (AC: 3)
  - [x] 在 BFF 中实现 `POST /api/media/uploads` 路由，接入本地/对象存储并返回路径与哈希，满足媒体上传规划。 [Source: docs/architecture/4-链下服务与数据流.md#4-7-2-媒体存证与上传接口-规划]
  - [x] 建立 `media_assets` 表与索引流程，将上传记录写入数据库（`matchedOffchain` 将在订单事件摄入时更新）。 [Source: docs/architecture/4-链下服务与数据流.md#4-2-数据模型-prisma-postgres]
  - [x] 编写后端/集成测试覆盖上传成功、哈希不一致、文件超限等路径，并在 Runbook 中补充分步巡检。 [Source: docs/runbook.md#8-测试策略]
...
- **API Specifications**: `/api/media/uploads` 返回的存储路径与 hash 需与事件比对；当前 BFF 已提供接口并校验哈希，`/api/orders/:recordUid`、`/api/orders/:recordUid/media-verify` 提供复核能力。 [Source: docs/architecture/4-7-2-媒体存证与上传接口-规划]
...
- **Outstanding Work**: 订单事件摄入仍需补充 `order_media_assets` 联动将 `matchedOffchain` 置为 true，以支撑时间线对账。 [Source: docs/architecture/4-链下服务与数据流.md#4-7-2-媒体存证与上传接口-规划]

### Testing
- **Framework & Commands**: 运行 `npm run test --workspace apps/web`、`npm run lint --workspace apps/web` 与 `npm run test --workspace apps/bff`，前端使用 Vitest + React Testing Library，后端使用 Jest。 [Source: docs/runbook.md#8-测试策略]
...
| 2025-09-18 | v0.4    | Added media upload BFF pipeline, Prisma model, and runbook steps | James |
...
- Delivered `/api/media/uploads` BFF pipeline with disk storage, Prisma persistence, repository layer, and integration/unit tests; documented media env variables and manual verification in runbook.
- Tests: `pnpm --filter @haigo/web lint`, `pnpm --filter @haigo/web test`, `pnpm --filter @haigo/bff test` (BFF lint still flags legacy issues in accounts module).
...
- apps/bff/src/modules/media/media.module.ts
- apps/bff/src/modules/media/media.controller.ts
- apps/bff/src/modules/media/media.service.ts
- apps/bff/src/modules/media/media.storage.ts
- apps/bff/src/modules/media/media.repository.ts
- apps/bff/src/modules/media/dto/upload-media.dto.ts
- apps/bff/src/shared/config/orders.ts
- apps/bff/src/shared/dto/orders.ts
- apps/bff/test/media.controller.spec.ts
- apps/bff/test/media.service.spec.ts
- docs/runbook.md
...
### Review Date: 2025-09-20 (Retest)

### Reviewed By: Quinn (Test Architect)

### Code Quality Assessment
- Re-verification counter now increments once per manual attempt, matching telemetry expectations (`apps/web/features/orders/inbound/OrderCheckInView.tsx:401-416`).
- Added regression coverage for manual re-check flow and throttled retries to guard against regressions.
...
- Coding Standards: ✓
- Project Structure: ✓
- Testing Strategy: ✓ — New tests cover manual re-check increments and failure throttle paths.
- All ACs Met: ✓ — Hash verification status, telemetry, and retry handling now satisfy tasks.

### Improvements Checklist
- [x] Ensure `updateVerificationStatus` only increments once per manual re-check (see diff above).
- [x] Add unit test covering manual re-check counter behaviour.

### Security Review
- PASS — No new attack surface; telemetry fix is UI state only.

### Performance Considerations
- Unchanged; hashing and upload flows unaffected by counter fix.
...
### Gate Status
Gate: PASS → docs/qa/gates/2.3-inbound-media-upload-hash-verification.yml
Risk profile: (not generated)
NFR assessment: (not generated)

### Recommended Status
✓ Ready for Merge — Story 2.3 meets QA expectations.
